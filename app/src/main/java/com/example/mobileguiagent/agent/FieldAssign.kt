...
/**
 * 어느 칸에 어느 값이 들어가는지 앱이 정한다. eval/assign.py를 그대로 옮긴 것이다.
 *
 * 왜 앱 안으로 옮기는가:
 *   지금까지 이 판단은 맥북의 파이썬이 했다. 그러려면 금고 값이 device_get_field로
 *   폰을 나와 맥북 메모리를 거쳐 다시 폰으로 들어와야 한다. 클라우드에 가지
 *   않을 뿐, "개인정보는 기기에서 처리한다"는 말과는 어긋난다. 판단이 앱 안에
 *   있으면 값이 앱 프로세스 밖으로 한 번도 나가지 않는다.
 *
 * 왜 모델이 아니라 규칙이 정하는가:
